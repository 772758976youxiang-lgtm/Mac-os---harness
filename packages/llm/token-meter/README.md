...
When the log's route is `deepseek-official`, the view also carries `cost` — a CNY estimate of the accumulated usage priced **per request at the time that request's usage was reported**, using DeepSeek's official per-model rates and peak/valley windows (peak 09:00–12:00 and 14:00–18:00 Beijing time; weekends all-day valley since 2026-08-23). Each sample is priced under the `request/context` route logged for that request, so a peak-hour request keeps its peak price even when the session runs on into valley hours; the estimate is a display figure, never a billing source, and it stays absent (with the `view` omitting `cost`) when no deepseek-official usage accumulated — consumers then fall back to their own heuristics instead of guessing a currency.
