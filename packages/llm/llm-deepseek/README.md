...
- **`ctx.<provider>.balance`** — the plugin provides `deepseek-official.balance`, the one service read by the host's `host.balance` RPC (the web UI's account-balance and session-cost rows). It reads `GET {baseURL}/user/balance` under the *same* resolved connection and API key stream calls use, so the displayed balance tracks the adapter's settings and credentials one-for-one; a non-ok response throws `LlmError('TRANSPORT')` with the provider detail, and a response without any derivable total throws `INVALID_RESPONSE`.
